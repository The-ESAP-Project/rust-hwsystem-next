...
pub async fn get_class_and_user_student_by_id_and_code(
    ...
    let row = sqlx::query(
        "SELECT c.*, cs.id, cs.class_id, cs.student_id, cs.role, cs.joined_at
        FROM classes c
    ...
    if let Some(row) = row {
        ...
        // BUG: class_student 在任何时候都是 None
        let class_student = row
            .try_get::<i64, _>("cs_id")
            .ok()
            .and_then(|_| ClassStudent::from_row(&row).ok());

        tracing::warn!(class = ?class, class_student = ?class_student);
