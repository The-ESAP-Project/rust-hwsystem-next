...
pub async fn list_users(
    ...
    USER_SERVICE
        .list_users(query.into_inner(), Some(&req))
        .await
}
...
pub async fn create_user(
    ...
    USER_SERVICE
        .create_user(user_data.into_inner(), Some(&req))
        .await
}

pub async fn get_user(req: HttpRequest, path: web::Path<i64>) -> ActixResult<HttpResponse> {
    USER_SERVICE.get_user(path.into_inner(), Some(&req)).await
}
...
pub async fn update_user(
    ...
    USER_SERVICE
        .update_user(path.into_inner(), update_data.into_inner(), Some(&req))
        .await
}

pub async fn delete_user(req: HttpRequest, path: web::Path<i64>) -> ActixResult<HttpResponse> {
    USER_SERVICE
        .delete_user(path.into_inner(), Some(&req))
        .await
}
